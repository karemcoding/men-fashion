...
class NestedSetsBehavior extends Behavior
{
     ...
     * @throws Exception
     */
    public function nestedSoftDelete()
    {
        if ($this->owner->isRoot()) {
            return $this->nestedSoftDeleteRoot();
        }
        return $this->nestedSoftDeleteNode();
    }
     ...
     * @throws Exception
     */
    public function nestedSoftDeleteNode()
    {
        $leftValue = $this->owner->getAttribute($this->leftAttribute);
        $rightValue = $this->owner->getAttribute($this->rightAttribute);
        ...
        try {
            ...
            $updateColumn = [
                ...
                $this->rightAttribute => null,
                $this->treeAttribute => null,
                $this->depthAttribute => null,
            ];
            $updated = $connection->createCommand()
     ...
     * @throws \Exception
     */
    protected function nestedSoftDeleteRoot()
    {
        $treeValue = $this->owner->getAttribute($this->treeAttribute);
        $connection = $this->owner->getDb();
        $transaction = $connection->beginTransaction();
        try {
            /** @var ActiveRecord $class */
            $class = get_class($this->owner);
            $updateColumn = ['status' => -10];
            $updated = $connection->createCommand()
                ->update($class::tableName(),
                    $updateColumn, $this->treeAttribute . '=:tree', [':tree' => $treeValue]
                )->execute();
            if ($updated > 0) {
                $transaction->commit();
                return true;
